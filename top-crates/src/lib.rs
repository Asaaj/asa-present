...
use cargo::core::compiler::{CompileKind, CompileTarget, TargetInfo};
...
use cargo::core::{Dependency, Package, PackageId, SourceId, Summary, Target};
...
impl Modifications {
	fn excluded(&self, name: &str) -> bool { self.exclusions.iter().any(|n| n == name) }
}
...
impl TopCrates {
	...
	fn add_rust_cookbook_crates(&mut self) {
		...
		resp.read_to_string(&mut content).expect("could not read cookbook manifest");

		let manifest = content.parse::<toml::Value>().expect("could not parse cookbook manifest");

		let dependencies = manifest["dependencies"]
			.as_table()
			.expect("no dependencies found for cookbook manifest");
		self.crates.extend({
			dependencies.iter().map(|(name, _)| Crate { name: InternedString::new(name) })
		})
	}

	/// Add crates that have been hand-picked
	fn add_curated_crates(&mut self, modifications: &Modifications) {
		self.crates.extend(modifications.additions.iter().copied().map(|name| Crate { name }));
	}
}
...
fn playground_metadata_features(pkg: &Package) -> Option<(BTreeSet<InternedString>, bool)> {
	...
	impl Default for Metadata {
		fn default() -> Self {
			Metadata { features: BTreeSet::new(), default_features: true, all_features: false }
		}
	}
...
fn make_global_state<'cfg>(
	...
	GlobalState { config, target_info, registry, crates_io, source, modifications }
}
...
fn populate_initial_direct_dependencies(
	...
	for download in packages {
		...
		let lib_target =
			download.library().unwrap_or_else(|| panic!("{} did not have a library", id)).clone();
	...
	for (name, pkgs) in &crates.iter().group_by(|dep| dep.summary.name()) {
		...
		for dep in pkgs {
			...
			let exposed_name = if first {
				...
				format!("{}_{}_{}_{}", crate_name, version.major, version.minor, version.patch)
			};
