...
use crate::coordinator::compile_error::CodeNotUtf8Snafu;
use crate::message::{
...
impl Edition {
	...
	pub(crate) fn to_cargo_toml_key(self) -> &'static str { self.to_str() }
}
...
impl CrateType {
	...
	pub(crate) fn is_binary(self) -> bool { self == CrateType::Binary }
...
impl CompileRequest {
	...
	pub(crate) fn postprocess_result(&self, code: CompiledCode) -> CompiledCode {
		if let CompiledCode::CodeStr(code_str) = &code {
			if let CompileTarget::Assembly(_, demangle, process) = self.target {
				if demangle == DemangleAssembly::Demangle {
					return CompiledCode::CodeStr(asm_cleanup::demangle_asm(code_str));
				}

				if process == ProcessAssembly::Filter {
					return CompiledCode::CodeStr(asm_cleanup::filter_asm(code_str));
				}
			}
		}
...
impl CompiledCode {
	pub fn from_bytes(source: Vec<u8>) -> Self {
		match String::from_utf8(source.clone()).context(CodeNotUtf8Snafu) {
			Ok(code_str) => Self::CodeStr(code_str),
			Err(_e) => Self::CodeBin(source),
		}
	}
}
...
impl Coordinator<DockerBackend> {
	pub async fn new_docker() -> Self { Self::new(DockerBackend(())).await }
}
...
impl Container {
	async fn new(
		...
		let task = tokio::spawn(
			async move {
				...
				d.context(DemultiplexerTaskPanickedSnafu)?.context(DemultiplexerTaskFailedSnafu)?;
				if let Some(t) = t {
		...
		Ok(Container { task, kill_child, modify_cargo_toml, commander })
	}
	...
	async fn versions(&self) -> Result<ChannelVersions, ContainerVersionsError> {
		...
		let rustfmt = rustfmt.context(RustfmtSnafu)?.context(RustfmtMissingSnafu)?;
	...
	async fn version_output(
		...
		let SpawnCargo { task, stdin_tx, stdout_rx, stderr_rx, status_rx } = v;
	...
	async fn execute(
		...
		let ActiveExecution { task, stdin_tx, stdout_rx, stderr_rx, status_rx } =
	...
	async fn begin_execute(
		...
		let SpawnCargo { task, stdin_tx, stdout_rx, stderr_rx, status_rx } =
			self.spawn_cargo_task(token, execute_cargo).await.context(CouldNotStartCargoSnafu)?;
		...
		Ok(ActiveExecution { task, stdin_tx, stdout_rx, stderr_rx, status_rx })
	}
	...
	async fn compile(
		...
		let ActiveCompilation { task, stdout_rx, stderr_rx } =
	...
	async fn begin_compile(
		...
		let read_output = ReadFileRequest { path: output_path.to_owned() };
		...
		let task = async move {
			...
			let code = if success {
				...
				CompiledCode::from_bytes(file.0)
			} else {
				CompiledCode::CodeStr(String::new())
			};
		...
		Ok(ActiveCompilation { task, stdout_rx, stderr_rx })
	}
	...
	async fn format(
		...
		let ActiveFormatting { task, stdout_rx, stderr_rx } =
	...
	async fn begin_format(
		...
		let write_main = request.write_main_request();
		let execute_cargo = request.execute_cargo_request();
		let read_output = ReadFileRequest { path: request.crate_type.primary_path().to_owned() };
		...
		Ok(ActiveFormatting { task, stdout_rx, stderr_rx })
	}
	...
	async fn clippy(
		...
		let ActiveClippy { task, stdout_rx, stderr_rx } = self.begin_clippy(token, request).await?;
	...
	async fn begin_clippy(
		...
		Ok(ActiveClippy { task, stdout_rx, stderr_rx })
	}

	async fn miri(&self, request: MiriRequest) -> Result<WithOutput<MiriResponse>, MiriError> {
		let token = Default::default();

		let ActiveMiri { task, stdout_rx, stderr_rx } = self.begin_miri(token, request).await?;
	...
	async fn begin_miri(
		...
		Ok(ActiveMiri { task, stdout_rx, stderr_rx })
	}
	...
	async fn macro_expansion(
		...
		let ActiveMacroExpansion { task, stdout_rx, stderr_rx } =
	...
	async fn begin_macro_expansion(
		...
		Ok(ActiveMacroExpansion { task, stdout_rx, stderr_rx })
	}
	...
	async fn spawn_cargo_task(
		...
		Ok(SpawnCargo { task, stdin_tx, stdout_rx, stderr_rx, status_rx })
	}

	async fn shutdown(self) -> Result<()> {
		let Self { task, kill_child, modify_cargo_toml, commander } = self;
		drop(commander);
...
impl Commander {
	...
	fn next_id(&self) -> JobId { self.id.fetch_add(1, Ordering::SeqCst) }
...
static DOCKER_BACKEND_START: Lazy<u64> = Lazy::new(|| {
	...
	now.duration_since(time::UNIX_EPOCH).unwrap_or_default().as_secs()
});
...
mod tests {
		...
		T: Backend,
	{
			...
			Fut: Future<Output = Coordinator<T>>,
		{
			...
			let permit = semaphore.acquire_owned().await.expect("Unable to acquire permit");
			let coordinator = f().await;
			Self { _permit: permit, coordinator }
		}
	...
	impl<T> ops::Deref for RestrictedCoordinator<T> {
		...
		fn deref(&self) -> &Self::Target { &self.coordinator }
	}

	impl<T> ops::DerefMut for RestrictedCoordinator<T> {
		fn deref_mut(&mut self) -> &mut Self::Target { &mut self.coordinator }
	}
	...
	async fn execute_stdin() -> Result<()> {
		...
		let token = Default::default();
		let ActiveExecution { task, stdin_tx, stdout_rx, stderr_rx, status_rx: _status_rx } =
	...
	async fn execute_stdin_close() -> Result<()> {
		...
		let token = Default::default();
		let ActiveExecution { task, stdin_tx, stdout_rx, stderr_rx, status_rx: _ } =
			coordinator.begin_execute(token, request).await.unwrap();
	...
	async fn execute_kill() -> Result<()> {
		...
		let token = CancellationToken::new();
		let ActiveExecution { task, stdin_tx: _, stdout_rx, stderr_rx, status_rx: _ } =
	...
	async fn execute_status() -> Result<()> {
		...
		let token = CancellationToken::new();
		let ActiveExecution { task, stdin_tx: _, stdout_rx, stderr_rx, status_rx } =
			coordinator.begin_execute(token.clone(), request).await.unwrap();
	...
	async fn memory_usage_is_limited() -> Result<()> {
		...
		assert!(!res.success);
		// TODO: We need to actually inform the user about this somehow. The UI is
		// blank. assert_contains!(res.stdout, "Killed");
